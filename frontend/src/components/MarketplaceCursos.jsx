...
const etapasCapacitacion = [
  {
    titulo: 'Diagnostica',
    texto: 'Identifica tu punto de partida y elige una ruta segun tu objetivo de negocio.',
  },
  {
    titulo: 'Aprende',
    texto: 'Avanza por modulos cortos con recursos descargables y actividades practicas.',
  },
  {
    titulo: 'Aplica',
    texto: 'Convierte cada leccion en una accion concreta para vender, ordenar o comunicar mejor.',
  },
];
...
export default function MarketplaceCursos() {
  ...
  // Save and notify when inscritos or progreso change
  useEffect(() => {
    try {
  ...
  const indicadoresCursos = useMemo(() => {
    const avances = cursos.map((curso) => progresoCursos[curso.id] ?? curso.avance);
    const avancePromedio = Math.round(
      avances.reduce((total, avance) => total + avance, 0) / cursos.length,
    );
    const cursosCompletados = avances.filter((avance) => avance === 100).length;
    const horasRegistradas = cursosInscritos.reduce((total, cursoId) => {
      const curso = cursos.find((item) => item.id === cursoId);
      const horas = Number.parseInt(curso?.duracion ?? '0', 10);

      return total + (Number.isNaN(horas) ? 0 : horas);
    }, 0);
    ...
    return {
      ...
      cursosCompletados,
      horasRegistradas,
      inscritos: cursosInscritos.length,
    };
  }, [cursosInscritos, progresoCursos]);

  const obtenerAvanceCurso = (curso) => progresoCursos[curso.id] ?? curso.avance;
  ...
  const inscribirseCurso = () => {
    setCursosInscritos((actuales) =>
      actuales.includes(cursoSeleccionado.id) ? actuales : [...actuales, cursoSeleccionado.id],
    );
    setProgresoCursos((actual) => ({
      ...actual,
      [cursoSeleccionado.id]: Math.max(actual[cursoSeleccionado.id] ?? 0, 10),
    }));
    setMensajeCurso(`Ya puedes comenzar "${cursoSeleccionado.titulo}". Tu avance inicial es 10%.`);
  };
  ...
  const completarActividad = () => {
    ...
    setProgresoCursos((actual) => {
      const avanceActual = actual[cursoSeleccionado.id] ?? cursoSeleccionado.avance;
      const nuevoAvance = Math.min(100, avanceActual + Math.ceil(100 / cursoSeleccionado.modulos));

      return {
        ...actual,
        [cursoSeleccionado.id]: nuevoAvance,
      };
    });
    setMensajeCurso(`Actividad registrada para "${cursoSeleccionado.titulo}".`);
  };

  const reiniciarCurso = () => {
    setProgresoCursos((actual) => ({
      ...actual,
      [cursoSeleccionado.id]: 0,
    }));
    setMensajeCurso(`Avance reiniciado para "${cursoSeleccionado.titulo}".`);
  };
    ...
    <div className="marketplace-cursos">
      <section className="mc-overview" aria-labelledby="ecosistema-title">
        ...
        <div className="mc-overview-panel" aria-label="Resumen de plataforma">
          <div>
            <strong>{indicadoresCursos.inscritos}</strong>
            <span>Cursos inscritos</span>
          </div>
          <div>
            <strong>{indicadoresCursos.avancePromedio}%</strong>
      ...
      <section className="mc-section mc-section-cursos" id="cursos" aria-labelledby="cursos-title">
        ...
        <div className="mc-metric-grid" aria-label="Indicadores de cursos">
          ...
          <div>
            ...
            <strong>{indicadoresCursos.horasRegistradas} horas</strong>
          </div>
          <div>
            <span>Cursos completados</span>
            <strong>{indicadoresCursos.cursosCompletados}</strong>
          </div>
        </div>

        <div className="mc-training-panel" aria-label="Proceso de capacitación">
          {etapasCapacitacion.map((etapa, indice) => (
        ...
        <div className="mc-learning-layout">
          <div className="mc-learning-main">
            ...
            <div className="mc-course-grid">
              {cursosFiltrados.map((curso) => {
                const avanceCurso = obtenerAvanceCurso(curso);
                const estaInscrito = cursosInscritos.includes(curso.id);
                ...
                return (
                  ...
                  >
                    <div className="mc-card-meta">
                      <span>{estaInscrito ? 'En curso' : curso.insignia}</span>
                    ...
                    <div className="mc-progress-label">
                      ...
                      <strong>{avanceCurso}% completado</strong>
                    </div>
                    <div className="mc-progress" aria-label={`Avance del curso ${avanceCurso}%`}>
                      <span style={{ width: `${avanceCurso}%` }} />
                    </div>
          ...
          <aside className="mc-course-detail" aria-labelledby="curso-detalle-title">
            ...
            <div className="mc-current-progress">
              <div className="mc-progress-label">
                <span>Avance actual</span>
                <strong>{obtenerAvanceCurso(cursoSeleccionado)}%</strong>
              </div>
              <div
                className="mc-progress"
                aria-label={`Avance actual ${obtenerAvanceCurso(cursoSeleccionado)}%`}
              >
                <span style={{ width: `${obtenerAvanceCurso(cursoSeleccionado)}%` }} />
              </div>
            </div>

            <div className="mc-lesson-list" aria-label="Actividades del curso">
              {cursoSeleccionado.aprendizajes.map((aprendizaje, indice) => (
                <div key={aprendizaje}>
                  <span>{indice + 1}</span>
                  <p>{aprendizaje}</p>
                </div>
            ...
            <div className="mc-course-actions">
              ...
              <button className="mc-outline-button" onClick={completarActividad} type="button">
                Completar actividad
              </button>
              <button className="mc-text-button" onClick={reiniciarCurso} type="button">
                Reiniciar avance
              </button>
            </div>
